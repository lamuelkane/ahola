...
import {tuttorschedulelesson, tutorrescheduledlesson, tutorcancelledlesson} from '../Templates/student'
...
const Tutorschedule = ({weeks}) => {
    ...
    const {timezone, firstname: name, _id : id} = tutor
      ...
      const sampletime = tutor.lessons.find(les => les.id === day.exist)
    const lesson = {
        id: uuidv4(),
        tutor: {
            id,
        ...
        <button onClick={async(e) => 
            ...
            try{
                ...
                await axios.post(`${sever}/api/users/lessonbooked`, {
                    template: tuttorschedulelesson(data.firstname, tutor.firstname, getlessonintimezone(lesson, data.timezone), lesson.rate, `https://ahola.ch/messages?convid=${tutor._id + data._id}&&name=${tutor.firstname}&&rcrid=${tutor._id}`),
                    email: data.email
                  })
    ...
    <div className={`p-3`}>
            <div className="bg-blue-50 text-sm margin padding items-center flex justify-between p-3">
                current lesson time {DateTime.fromISO(getlessonintimezone(tutor.lessons.find(les => les.id === day.exist), tutor?.timezone), { zone: tutor?.timezone?.name }).toFormat('ff')}
                </div>
        ...
        <button onClick={async(e) => 
            ...
            try{
                ...
                await axios.post(`${sever}/api/users/lessonrescheduled`, {
                    template: tutorrescheduledlesson(tutor.firstname, data.firstname, getlessonintimezone(sampletime, data.timezone), getlessonintimezone(les, data.timezone), `https://ahola.ch/messages?convid=${tutor._id + data._id}&&name=${tutor.firstname}&&rcrid=${tutor._id}`),
                    email: data.email
                  })
                localStorage.setItem('user', JSON.stringify(tutor))
                Notification({
            ...
            try{
                ...
                if (les) {
                    ...
                    await axios.post(`${sever}/api/users/lessoncancelled`, {
                        template: tutorcancelledlesson(tutor.firstname, data.firstname, getlessonintimezone(les, data.timezone), `https://ahola.ch/messages?convid=${tutor._id + data._id}&&name=${tutor.firstname}&&rcrid=${tutor._id}`),
                        email: data.email
                      })
                    localStorage.setItem('user', JSON.stringify(tutor))
                    Notification({
