...
import {studentrescheduleslesson, studentschedulelesson, studentcancelledlesson} from '../Templates/tutor'
...
const Studentschedule = ({weeks}) => {
    ...
    const sampletime = student?.lessons.find(les => les.id === day.exist)
    const sample2 = sampletime
            ...
            try{
                ...
                if(exist || exist == 0){
                    ...
                    const sample =  await axios.post(`${sever}/api/users/lessonbooked`, {
                        template: studentschedulelesson(data.firstname, student.firstname, getlessonintimezone(lesson, data.timezone), lesson.rate, `https://ahola.ch/messages?convid=${data._id + student._id}&&name=${student.firstname}&&rcrid=${student._id}`),
                        email: data.email
                    })
 ...
 : 
    ...
    <div className="bg-blue-50 text-sm margin padding items-center flex justify-between p-3">
           current lesson time {DateTime.fromISO(getlessonintimezone(student?.lessons.find(les => les.id === day.exist), student?.timezone), { zone: student?.timezone?.name }).toFormat('ff')}
        </div>
        ...
        <button onClick={async(e) => 
            ...
            try{
                ...
                await axios.post(`${sever}/api/users/lessonrescheduled`, {
                    template: studentrescheduleslesson(data.firstname,
                        student.firstname,
                        DateTime.fromISO(getlessonintimezone(les, data?.timezone), { zone: data.timezone.name}).toFormat('ff'),
                        DateTime.fromISO(getlessonintimezone(sampletime, data.timezone), { zone: data.timezone.name}).toFormat('ff'),
                        `https://ahola.ch/messages?convid=${data._id + student._id}&&name=${student.firstname}&&rcrid=${student._id}`
                        ),
                    email: data.email
                  })
                localStorage.setItem('user', JSON.stringify(student))
            ...
            try{
               ...
               if (les) {
                ...
                await axios.post(`${sever}/api/users/lessoncancelled`, {
                    template: studentcancelledlesson(student.firstname, data.firstname, getlessonintimezone(sampletime, data.timezone), `https://ahola.ch/messages?convid=${data._id + student._id}&&name=${student.firstname}&&rcrid=${student._id}`),
                    email: data.email
                  })
                localStorage.setItem('user', JSON.stringify(student))
                Notification({
