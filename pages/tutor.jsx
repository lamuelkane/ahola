...
const Tutor = () => {
    ...
    const {sever, user, sever3} = useSelector((state) => state);
        ...
        <div>
            ...
            tutor.firstname  && <div>
                <div className={`${styles.tutoriframeholdeer}`}>
                    {
                        tutor?.video?.includes(sever3) ? <video controls className={`${styles.tutoriframe}`}  src={tutor?.video}></video>
                        :
                    <iframe className={`${styles.tutoriframe}`} src={`${ checkvideolink(tutor.video)? `https://www.youtube.com/embed/${tutorvideo()}` : `https://player.vimeo.com/video/${tutorvideo()}`}`}></iframe>
